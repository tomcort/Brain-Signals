...
def TCoherency(data, fs_welch=20, points_interval=300, nps = 250, Tnps = 100, nover = 230 , Tnover = 90, stat = True, Toverlap = 270):
    ...
    for i in range(Nt-1):
        ...
        if (Tf1!=Tf).any():
            return True
    data_i = data[:, TT - S:]
    Tcoherency[:,:,:,Nt-1] = Coherency(data_i, fs_welch=fs_welch, nps = Tnps, nover = Tnover)[1]
    Return +=  (Tf, Tcoherency)

    return Return  

def savesplitdata(data, name):
    i = 0
    for col in np.split(data, data.shape[1], axis=1):
        np.save(f'{name}{i}', col)
        i+=1

def loadsplitdata(name, ncols):
    arr = np.zeros(ncols, object)
    name = name.split(".")[0]
    for i in range(ncols):
        path = f"{name}{i}.npy" 
        arr[i] = np.load(path)[:,0,:,:]
    arr = np.stack(arr)
    sh = len(arr.shape)
    axs = [1,0] + list(range(2,sh))  
    arr = np.transpose(arr, axes = axs)
    return arr
